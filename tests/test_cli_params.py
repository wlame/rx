...
class TestCLISamples:
    ...
    def test_samples_basic(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--samples'])
    ...
    def test_samples_with_context(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--samples', '--context', '1'])
        assert result.exit_code == 0
    ...
    def test_samples_with_before_after(self):
        ...
        result = self.runner.invoke(
            trace_command, ['error', self.test_file, '--samples', '--before', '2', '--after', '1']
        )
        assert result.exit_code == 0
    ...
    def test_samples_with_json(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--samples', '--json'])
        assert result.exit_code == 0
    ...
    def test_samples_with_no_color(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--samples', '--no-color'])
        assert result.exit_code == 0
    ...
    def test_samples_with_directory(self):
        ...
        result = self.runner.invoke(trace_command, ['error', dir_path, '--samples'])
        assert result.exit_code == 0
        assert 'Path: ' in result.output
    ...
    def test_samples_with_directory_json(self):
        ...
        result = self.runner.invoke(trace_command, ['error', dir_path, '--samples', '--json'])
        assert result.exit_code == 0
    ...
    def test_samples_with_max_results(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--samples', '--max-results', '1'])
        assert result.exit_code == 0
...
class TestCLIPositionalAndNamedParams:
    ...
    def test_positional_params(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file])
        assert result.exit_code == 0
    ...
    def test_mixed_positional_and_named(self):
        ...
        # Positional pattern, positional path
        result = self.runner.invoke(trace_command, ['error', self.test_file])
        assert result.exit_code == 0
...
class TestCLIMaxResults:
    ...
    def test_max_results_limits_output(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--max-results', '3', '--json'])
        assert result.exit_code == 0
        data = json.loads(result.output)
    ...
    def test_max_results_without_limit(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--json'])
        assert result.exit_code == 0
        data = json.loads(result.output)
...
class TestCLIRipgrepPassthrough:
    ...
    def test_ignore_case_flag(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '-i', '--json'])
        assert result.exit_code == 0
        data = json.loads(result.output)
    ...
    def test_case_sensitive_flag(self):
        ...
        result = self.runner.invoke(trace_command, ['error', self.test_file, '--case-sensitive', '--json'])
        assert result.exit_code == 0
        data = json.loads(result.output)
    ...
    def test_multiple_passthrough_flags(self):
        ...
        result = self.runner.invoke(trace_command, ['error', test_file2, '-w', '--json'])
        assert result.exit_code == 0
        data = json.loads(result.output)
